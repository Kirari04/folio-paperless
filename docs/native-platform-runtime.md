...
Android uses the framework Platform AppSearch service, not an app-local AndroidX database. It is supported only on Android 12/API 31 and newer. The `FolioDocument` schema is explicitly displayed by the system and contains only the title property. Opaque profile/document identity is confined to AppSearch's required namespace and document ID. Devices below API 31 and devices without the platform service return a stable unsupported reason.
