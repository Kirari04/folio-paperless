...
/** A small incremental SHA-256 implementation used to avoid loading document files into memory. */
export class Sha256 {
  ...
  update(bytes: Uint8Array) {
    ...
    if (!Number.isSafeInteger(this.bytesHashed)) {
      throw new Error('The SHA-256 input is too large.');
    }

    let offset = 0;
    while (offset < bytes.byteLength) {
      if (this.bufferLength === 0 && bytes.byteLength - offset >= SHA256_BLOCK_BYTES) {
  ...
  private finish() {
    ...
    for (let index = 0; index < 4; index += 1) {
      this.buffer[56 + index] = high >>> (24 - index * 8);
      this.buffer[60 + index] = low >>> (24 - index * 8);
    }
    this.processBlock(this.buffer, 0);
    this.bufferLength = 0;
    this.finished = true;
...
export function sha256HexChunks(chunks: Iterable<Uint8Array>) {
